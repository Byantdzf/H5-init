      ...
      <div>
        <div v-for="item,index in list" class="main-box">
          <div class="main-info colorff">
            ...
            <div class="font28 content color6">{{item.content}}</div>
          ...
          <div class="comment" v-if="comments.length > 0">
            <div class="comment-box" v-for="itemComments in comments" @click="routeToDetail(itemComments.user.type, itemComments.user.id)">
              ...
              <span class="bold color6 font32">{{itemComments.user? itemComments.user.name: '未获取到用户信息'}}：
  ...
  export default {
    ...
    methods: {
      ...
      saveComment () { // 评论
        ...
        this.$http.post(`/official/comment/moments/${this.id}`, data).then(({data}) => {
          this.comment = ''
          $toastSuccess('评论成功')
          let page = {num: 1}
      ...
      getOrderList (page, mescroll) { // 获取数据
        ...
        let vm = this
        vm.$http.get(`/official/moments/${this.id}?page=${pageV}&type=${vm.searchType}`).then(({data}) => {
